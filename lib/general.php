...
class Consultas {
	...
	//obtener noticias
	/* esta funcion la cree para el rss, pero ahora creares el rss a partir del blog
	private $noticia = array();
	public  function getNoticias() {
		$query = "SELECT * FROM `platafor_web`.`noticias` ORDER BY idnoticia DESC;";
		$noticias = mysql_query($query, Consultas::conexionbd());
		while ($filas = mysql_fetch_assoc($noticias)) {
			$this->noticia[] = $filas;
		}
		return $this->noticia;
	}
