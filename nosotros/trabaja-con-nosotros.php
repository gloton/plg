		...
		<div class="two_column">      	
			<div class="left_col">
				<div class="contact-form-wrap">
                    ...
                    <p>En Plataformagroup estamos en constante búsqueda de los mejores profesionales 
                    para potenciar nuestras áreas de negocios. Si quieres integrarte a un equipo 
                    de profesionales con basta experiencia, encontrar un excelente clima laboral 
                    y muchas posibilidades de crecimiento profesional, ingresa tus datos y tu curriculum 
                    en el formulario de la derecha o envíanos tu curriculum a <a href="mailto:contacto@plataformagroup.cl" style="color: #1F9D48;">contacto@plataformagroup.cl</a>
                    </p>					
				</div>                
...
<!--jgl 14/05/2012 inicio formulario-->	
	<div id="form">   	    
        ...
        <? if ($flag!='ok') { ?>
        <form id="fmrCurriculum" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
        	<fieldset> 
            <p>
            <label for="nombre">Nombre *</label>
            <input <? if (isset ($flag) && $_POST['nombre']=='') { echo 'class="error"';} else {echo 'class="campo"';} ?> type="text" name="nombre" value="<? echo $_POST['nombre'];?>" onkeypress="return validar(event)"  />
            </p>
            <p>
            <label for="email">Email *</label>
            <input onblur="valida_correo(document.getElementById('email').value);" <?php if (isset ($flag) && $_POST['email']=='') { echo 'class="error"';} else {echo 'class="campo"';} ?> type="text" name="email" id="email" value="<? echo $_POST['email'];?>" />
            </p>
            <p>
            <label for="fono">Fono de Contacto *</label>
            <input <? if (isset ($flag) && $_POST['fono']=='') { echo 'class="error"';} else {echo 'class="campo"';} ?> type="text" name="fono" value="<? echo $_POST['fono'];?>"  />
            </p>
            <p>
            <label for="areaInteres">Área de Interes *</label>
            <input <? if (isset ($flag) && $_POST['areaInteres']=='') { echo 'class="error"';} else {echo 'class="campo"';} ?> type="text" name="areaInteres" value="<? echo $_POST['areaInteres'];?>" onkeypress="return validar(event)"  />
            </p>                        
            <p>
            <label for="archivo">Subir Archivo* (.doc, .pdf)</label>
            <input id="bntExaminar" <? if (isset ($flag) && $ext=='error' || $tamano > $maximo_tamano) { echo 'class="error"';} else {echo 'class="campo"';} ?> type="file" name="archivo"  value="<? echo $_FILE['archivo'];?>" />
            </p>
            <p>
            ...
            <p><input class="boton" type="submit" name="enviar" value="enviar" />
            </p>
            </fieldset>
		</form>
        <? } ?>
